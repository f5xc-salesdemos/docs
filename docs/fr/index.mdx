---
title: Démos commerciales F5 Distributed Cloud
description: >-
  Guides de démonstration et runbooks pour l'ingénierie commerciale F5
  Distributed Cloud.
template: splash
hero:
  title: Sales Demos
  tagline: Demo guides and runbooks for F5 Distributed Cloud sales engineering.
  image:
    html: >-
      <img src="/docs/images/hero-platform.svg" alt="F5 Distributed Cloud
      platform" />
i18n:
  sourceHash: 9502e0988fe3
  translator: machine
---

import { CardGrid } from '@astrojs/starlight/components';
import LinkCard from '@f5xc-salesdemos/docs-theme/components/LinkCard.astro';

## Solutions de sécurité

<CardGrid>
  <LinkCard
    icon="f5xc:web-app-and-api-protection"
    title="WAF"
    description="Configuration du pare-feu applicatif web et stratégies."
    href="https://f5xc-salesdemos.github.io/waf/"
  />
  <LinkCard
    icon="f5xc:web-app-and-api-protection"
    title="API"
    description="Découverte d'API, protection et stratégies de sécurité."
    href="https://f5xc-salesdemos.github.io/api-protection/"
  />
  <LinkCard
    icon="f5xc:bot-defense"
    title="Bot Advanced"
    description="Défense avancée contre les bots avec analyse comportementale."
    href="https://f5xc-salesdemos.github.io/bot-advanced/"
  />
  <LinkCard
    icon="f5xc:bot-defense"
    title="Bot Standard"
    description="Défense standard contre les bots avec détection basée sur les signatures."
    href="https://f5xc-salesdemos.github.io/bot-standard/"
  />
  <LinkCard
    icon="f5xc:client-side-defense"
    title="CSD"
    description="Défense côté client contre les menaces basées sur les navigateurs."
    href="https://f5xc-salesdemos.github.io/csd/"
  />
  <LinkCard
    icon="f5xc:ddos-and-transit-services"
    title="DDoS"
    description="Protection contre les attaques par déni de service distribué."
    href="https://f5xc-salesdemos.github.io/ddos/"
  />
  <LinkCard
    icon="f5xc:web-app-scanning"
    title="WAS"
    description="Scan d'applications web et évaluation des vulnérabilités."
    href="https://f5xc-salesdemos.github.io/was/"
  />
</CardGrid>

## Réseau et performance

<CardGrid>
  <LinkCard
    icon="f5xc:multi-cloud-network-connect"
    title="MCN"
    description="Réseautage multi-cloud et connectivité de sites."
    href="https://f5xc-salesdemos.github.io/mcn/"
  />
  <LinkCard
    icon="f5xc:content-delivery-network"
    title="CDN"
    description="Réseau de distribution de contenu et mise en cache en périphérie."
    href="https://f5xc-salesdemos.github.io/cdn/"
  />
  <LinkCard
    icon="f5xc:dns-management"
    title="DNS"
    description="Gestion DNS, zones et équilibrage de charge."
    href="https://f5xc-salesdemos.github.io/dns/"
  />
  <LinkCard
    icon="f5xc:nginx-one"
    title="NGINX"
    description="Intégration NGINX et gestion de configuration."
    href="https://f5xc-salesdemos.github.io/nginx/"
  />
</CardGrid>

## Ressources de démonstration

<CardGrid>
  <LinkCard
    icon="f5xc:distributed-apps"
    title="Serveur d'origine"
    description="VM Ubuntu 24.04 avec 9 applications web vulnérables pour les tests WAF, API et défense contre les bots."
    href="https://f5xc-salesdemos.github.io/origin-server/"
  />
  <LinkCard
    icon="f5xc:application-traffic-insight"
    title="Générateur de trafic"
    description="VM Azure avec 50+ outils de sécurité et 7 suites d'attaque pour la génération de trafic de démonstration."
    href="https://f5xc-salesdemos.github.io/traffic-generator/"
  />
  <LinkCard
    icon="f5xc:content-delivery-network"
    title="Simulateur CDN"
    description="Simulateur de nœud périphérique CDN basé sur NGINX avec 67+ en-têtes spécifiques aux fournisseurs."
    href="https://f5xc-salesdemos.github.io/cdn-simulator/"
  />
  <LinkCard
    icon="f5xc:platform"
    title="Afficher tous les composants"
    description="Parcourez le catalogue complet des ressources de démonstration."
    href="https://f5xc-salesdemos.github.io/demo-resources/"
  />
</CardGrid>

## Opérations

<CardGrid>
  <LinkCard
    icon="f5xc:observability"
    title="Observabilité"
    description="Surveillance, métriques et insights applicatifs."
    href="https://f5xc-salesdemos.github.io/observability/"
  />
  <LinkCard
    icon="f5xc:administration"
    title="Administration"
    description="Gestion des locataires, RBAC et paramètres de plateforme."
    href="https://f5xc-salesdemos.github.io/administration/"
  />
</CardGrid>

## Automatisation

<CardGrid>
  <LinkCard
    icon="hashicorp-flight:terraform-color"
    title="Fournisseur Terraform"
    description="Fournisseur Terraform pour F5 Distributed Cloud."
    href="https://f5xc-salesdemos.github.io/terraform-provider-f5xc/"
  />
  <LinkCard
    icon="f5xc:data-intelligence"
    title="Spécifications API"
    description="Validation des spécifications OpenAPI et réconciliation pour F5 Distributed Cloud."
    href="https://f5xc-salesdemos.github.io/api-specs/"
  />
  <LinkCard
    icon="f5xc:data-intelligence"
    title="Spécifications API enrichies"
    description="Spécifications OpenAPI enrichies pour F5 Distributed Cloud."
    href="https://f5xc-salesdemos.github.io/api-specs-enriched/"
  />
</CardGrid>

## Plateforme et outils

<CardGrid>
  <LinkCard
    icon="f5xc:doc"
    title="Builder"
    description="Système de construction de documentation Astro + Starlight conteneurisé."
    href="https://f5xc-salesdemos.github.io/docs-builder/"
  />
  <LinkCard
    icon="f5xc:shared-configuration"
    title="Thème"
    description="Marque partagée et style pour les sites de documentation."
    href="https://f5xc-salesdemos.github.io/docs-theme/"
  />
  <LinkCard
    icon="f5xc:distributed-apps"
    title="Icônes"
    description="Packages d'icônes NPM et plugins pour le système de construction de documentation."
    href="https://f5xc-salesdemos.github.io/docs-icons/"
  />
  <LinkCard
    icon="f5xc:shared-configuration"
    title="Contrôle"
    description="Workflows CI, modèles de gouvernance et application des paramètres de dépôt."
    href="https://f5xc-salesdemos.github.io/docs-control/"
  />
  <LinkCard
    icon="hashicorp-flight:docker-color"
    title="Conteneur de développement"
    description="Environnement de développement isolé avec outils de codage IA."
    href="https://f5xc-salesdemos.github.io/devcontainer/"
  />
</CardGrid>

## Outils pour développeurs

<CardGrid>
  <LinkCard
    icon="f5xc:distributed-apps"
    title="Extension VS Code"
    description="Gérez les ressources F5 XC depuis VS Code avec IntelliSense et l'assistant xcsh chat."
    href="https://f5xc-salesdemos.github.io/vscode-f5xc-tools/"
  />
  <LinkCard
    icon="f5xc:ai_assistant_logo"
    title="CLI xcsh"
    description="Shell alimenté par l'IA pour les opérations F5 Distributed Cloud."
    href="https://f5xc-salesdemos.github.io/xcsh/"
  />
</CardGrid>

## IA

<CardGrid>
  <LinkCard
    icon="f5xc:ai_assistant_logo"
    title="Marketplace"
    description="Marketplace alimentée par l'IA pour les solutions F5 Distributed Cloud."
    href="https://f5xc-salesdemos.github.io/marketplace/"
  />
</CardGrid>
